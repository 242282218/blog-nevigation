...
正式发布建议使用 GitHub Actions 的 `Docker Build & Publish` workflow。workflow 会先用 Buildx 推送临时候选镜像，再按候选 digest 拉取同一产物，挂载临时数据目录验证 `/var/lib/blog-navigation` 可持久写入，并对同一 digest 执行 Trivy HIGH/CRITICAL 镜像扫描；通过后才把该 digest promote 到 GHCR 稳定标签。
...
当前自动 R2 备份优先使用 v2 分块格式，`v2/latest.json` 指向最新快照；恢复链路仍兼容旧版 v1 `latest/backup.json`。
...
`/api/health` 是轻量存活检查，只验证进程能响应和运行时数据目录可写，适合负载均衡或外部监控判断实例是否可服务。

`/api/ready` 是深度就绪检查，会额外验证 manifest 和 R2 远端备份队列；如果远端备份失败但本地站点仍可读写，`/api/ready` 会返回 degraded，`/api/health` 仍可保持 ok。
