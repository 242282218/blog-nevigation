...
type CloudflareR2Form = Omit<CloudflareR2Settings, 'hasSecretAccessKey' | 'hasAccessKeyId' | 'hasBackupEncryptionPassphrase'> & {
    accessKeyId: string;
...
type CloudflareR2ValidationField = 'accountId' | 'bucket' | 'accessKeyId' | 'secretAccessKey' | 'backupEncryptionPassphrase';
type CloudflareR2BootstrapValidationField = 'authEmail' | 'globalApiKey' | 'accountId' | 'bucket' | 'backupEncryptionPassphrase';
...
function validateCloudflareR2Form(
    ...
    if (!form.backupEncryptionPassphrase.trim() && !hasBackupEncryptionPassphrase) {
        return { field: 'backupEncryptionPassphrase', message: '请填写备份加密口令。' };
    }
...
function validateCloudflareR2BootstrapForm(form: CloudflareR2BootstrapForm): CloudflareR2BootstrapValidationError | null {
    ...
    if (!form.backupEncryptionPassphrase.trim()) {
        return { field: 'backupEncryptionPassphrase', message: '请填写备份加密口令。' };
    }
...
const CLOUDFLARE_R2_FIELD_IDS: Record<CloudflareR2ValidationField, string> = {
    ...
    backupEncryptionPassphrase: 'r2-backup-encryption-passphrase',
};
const CLOUDFLARE_R2_BOOTSTRAP_FIELD_IDS: Record<CloudflareR2BootstrapValidationField, string> = {
    ...
    backupEncryptionPassphrase: 'r2-bootstrap-backup-encryption-passphrase',
};

function isCloudflareR2ValidationField(id: keyof CloudflareR2Form): id is CloudflareR2ValidationField {
    return id === 'accountId' || id === 'bucket' || id === 'accessKeyId' || id === 'secretAccessKey' || id === 'backupEncryptionPassphrase';
}
...
export function CloudflareR2SettingsPanel() {
    ...
    const [hasBackupEncryptionPassphrase, setHasBackupEncryptionPassphrase] = useState(false);
        ...
        async function loadCloudflareR2Settings() {
            try {
                ...
                if (isMounted) {
                    ...
                    setHasBackupEncryptionPassphrase(Boolean(payload?.settings?.hasBackupEncryptionPassphrase));
    ...
    const handleSubmit = useCallback(
        ...
        [form, hasSecretAccessKey, hasBackupEncryptionPassphrase]
    );
    ...
    return (
        <EditorPanel className="p-4 lg:col-span-2">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-sm leading-6 text-muted">
                                配置远端灾备镜像。R2 备份会以 AES-256-GCM 加密 JSON 保存。
                            </p>
                        </div>
                    </div>
                ...
                <div className="mb-4 grid grid-cols-2 gap-3 rounded-token-card border border-border-soft bg-background p-3 text-sm md:grid-cols-6">
                    <div>
                    ...
                    <div>
                        <p className="font-mono text-xs text-subtle">passphrase</p>
                        <p className="mt-1 text-muted">{hasBackupEncryptionPassphrase ? '已保存' : '未保存'}</p>
                    </div>
                    <div>
                        <p className="font-mono text-xs text-subtle">backup</p>
                        <p className="mt-1 text-muted">加密 JSON</p>
                    </div>
                </div>
                ...
                <div className="mb-4 rounded-token-card border border-border-soft bg-background p-3 text-sm leading-6 text-muted">
                    ...
                    <ul className="mt-2 list-disc space-y-1 pl-5">
                        ...
                        <li>运行时数据目录用于保存本页配置；R2 备份会以加密 JSON 写入对象存储。</li>
                    </ul>
                </div>
                ...
                <div className="grid gap-4">
                    <div className="grid gap-4 rounded-token-card border border-border-soft bg-background p-3">
                        <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-subtle">
                                Global API Key 不会写入服务器配置；配置完成后会保存 bucket 专用 R2 凭证和备份加密口令。
                            </p>
                        </div>
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            />
                            <TextField
                                id="r2-bootstrap-backup-encryption-passphrase"
                                label="备份加密口令"
                                help="用于加密 R2 备份；丢失后无法解密已有 R2 备份。"
                                value={bootstrapForm.backupEncryptionPassphrase}
                                type="password"
                                error={bootstrapFieldErrors.backupEncryptionPassphrase}
                                onChange={(value) => updateBootstrapField('backupEncryptionPassphrase', value)}
                            />
                            <TextField
                    ...
                    >
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            />
                            <TextField
                                id="r2-backup-encryption-passphrase"
                                label="备份加密口令"
                                help={hasBackupEncryptionPassphrase ? '已保存加密口令；留空会继续使用已保存值。' : '用于加密 R2 备份，首次启用必填。'}
                                value={form.backupEncryptionPassphrase}
                                type="password"
                                placeholder={hasBackupEncryptionPassphrase ? '已保存，留空不变' : ''}
                                error={fieldErrors.backupEncryptionPassphrase}
                                onChange={(value) => updateField('backupEncryptionPassphrase', value)}
                            />
                            <TextField
                                id="r2-prefix"
                                label="对象前缀"
